  ...
  return <>
    ...
    <header className="site-header" onKeyDown={event => { if (event.key === "Escape") close(); }}>
      <div className="header-inner">
        <Link href="/" className="brand" onClick={() => setOpen(false)}><Image src="/logo.png" alt="SolarWise" width={70} height={44} className="brand-logo" priority /><span>SolarWise</span></Link>
        <nav id="primary-nav" aria-label={ar ? "التنقل الرئيسي" : "Main navigation"} className={`navigation ${open ? "is-open" : ""}`}>
          <div className="drawer-head">
            <span className="drawer-brand"><Image src="/logo.png" alt="SolarWise" width={60} height={38} className="brand-logo" />SolarWise</span>
          </div>
