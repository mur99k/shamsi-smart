...
export function TimeField({ value, label, onChange }: { value: string; label: string; onChange: (value: string) => void }) {
  ...
  return <label className="time-field">{label}<input type="text" inputMode="numeric" dir="ltr" lang="en" aria-label={label} placeholder="12:30" maxLength={5} value={draft ?? value} onChange={event => setDraft(event.target.value)} onBlur={event => commit(event.target.value)} onKeyDown={event => { if (event.key === "Enter") (event.target as HTMLInputElement).blur(); }} /></label>;
}
  ...
  return <section className="section sim-inputs"><h2>{ar ? "مدخلات المحاكاة" : "Simulation inputs"}</h2>
    ...
    <details className="acc" open><summary>{ar ? "🔌 الأجهزة والأحمال" : "🔌 Devices & loads"}</summary>
      ...
      <fieldset><legend>{ar ? "الأجهزة المتاحة" : "Device availability"}</legend>
        ...
        <label className="check-row"><input type="checkbox" checked={sim.evAvailable} onChange={event => onChange({ evAvailable: event.target.checked, evCharging: event.target.checked ? sim.evCharging : false })} /><span>{ar ? "السيارة الكهربائية متاحة" : "EV available"}</span></label>
        {sim.evAvailable && <label className="check-row"><input type="checkbox" checked={sim.evCharging} onChange={event => onChange({ evCharging: event.target.checked })} /><span>{ar ? "السيارة قيد الشحن" : "EV already charging"}</span></label>}
      </fieldset>
